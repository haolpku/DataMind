...
def _load_clean_prompt() -> str:
    prompt_path = Path(__file__).resolve().parent.parent / "prompts" / "Data_clean_prompt.txt"
...
def process_wrapper(input_file, output_root, input_root):
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = {executor.submit(process_wrapper, f, output_root, input_root): f for f in json_files}
