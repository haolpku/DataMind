...
│   ├── graph_rag.py       # GraphRAG: 知识图谱构建 + 查询
│   ├── database.py        # Database: SQLite 示例数据库 + NL2SQL
│   ├── skills.py          # Skills: 自定义工具 (计算器/时间/换算等)
│   ├── memory.py          # Memory: 对话记忆管理
...
编辑 `rag/indexer.py` 中的 `build_index()` 函数：
...
编辑 `rag/graph_rag.py` 中的 `build_graph_index()` 函数：
...
同时更新 `rag/agent.py` 中 `database_query` 工具的 `description`，告诉 Agent 你的数据库有哪些表和字段，这样 Agent 才能正确生成 SQL。
