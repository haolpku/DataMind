...
> 数据预处理人员请参阅 **[data.md](docs/data.md)** — 说明各模块的数据格式规范和一键导入方式。
>
> 快速体验请参阅 **[demo.md](docs/demo.md)** — 精心设计的示例问题，演示各模块如何被 Agent 自动调用。
>
> 并发测评请参阅 **[benchmark.md](docs/benchmark.md)** — 说明如何下载真实 RAG 数据集并跑 benchmark。
...
```bash
SIMILARITY_TOP_K=3        # 返回最相关的文档块数量（增大可提高召回率）
RETRIEVER_MODE=simple     # 检索策略: simple (单 query) / multi_query (LLM 拆解子查询)
